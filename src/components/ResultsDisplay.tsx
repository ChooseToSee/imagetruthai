...
const ResultsDisplay = ({ result, imagePreview, onReset, streamProgress, partialReady, onKeepWaiting }: ResultsDisplayProps) => {
  ...
  const handleDownloadPdf = useCallback(() => {
    // Generate a text-based report and download as .txt (PDF would require a library)
    const lines = [
      ...
      `Models Analyzed: ${totalModels}`,
      `Model Agreement: ${modelsAgreed}/${totalModels}`,
      "",
      "Analysis Details:",
    ...
    lines.push(
      ...
      "",
      "Generated by ImageTruth AI — https://imagetruthai.lovable.app",
    );
    ...
    a.href = url;
    a.download = "imagetruth-report.txt";
    a.click();
    URL.revokeObjectURL(url);
    toast({ title: "Report downloaded", description: "Your analysis report has been saved." });
  }, [result, isAI, confidenceLabel, totalModels, modelsAgreed, manipulation, isEdited, toast]);
