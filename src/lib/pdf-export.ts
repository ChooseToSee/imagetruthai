...
export async function exportReportPdf(
  ...
  // Safari on macOS blocks pdf.save() blob downloads; use window.open fallback
  const isSafariDesktop = /^((?!chrome|android).)*safari/i.test(navigator.userAgent) && !/iPhone|iPad|iPod/.test(navigator.userAgent);
  if (isSafariDesktop) {
